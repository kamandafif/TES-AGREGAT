...
def buat_pdf_rekomendasi(ringkasan_teks: dict, gambar_png_bytes, catatan_rekomendasi: str):
    ...
    try:
        ...
        # ---- Ringkasan kondisi: SATU KOLOM PENUH (bukan tabel dua kolom) ----
        # Tata letak dua kolom (cell lebar tetap + cell sisa lebar) rawan
        # memicu FPDFException "Not enough horizontal space" pada fpdf2,
        # terutama setelah elemen lain (gambar) mengubah posisi kursor.
        # multi_cell(0, ...) jauh lebih aman: selalu memakai lebar penuh
        # halaman dan otomatis reset ke margin kiri setiap ganti baris.
        pdf.set_text_color(20, 20, 20)
        pdf.set_font("Helvetica", "B", 12)
        pdf.multi_cell(0, 8, bersihkan_teks_pdf("Ringkasan Kondisi Terdeteksi"))
        pdf.set_x(pdf.l_margin)
        pdf.set_font("Helvetica", "", 11)
        for label, value in ringkasan_teks.items():
            baris = bersihkan_teks_pdf(f"{label}: {value}")
            pdf.multi_cell(0, 7, baris)
            pdf.set_x(pdf.l_margin)
        pdf.ln(4)
        pdf.set_x(pdf.l_margin)
        ...
        if gambar_png_bytes:
            ...
            try:
                ...
                # x dikunci ke margin kiri secara eksplisit sebelum DAN
                # sesudah gambar disisipkan, supaya posisi kursor pasti
                # bersih untuk elemen berikutnya (jangan andalkan ln()
                # saja untuk mereset posisi).
                pdf.set_x(pdf.l_margin)
                pdf.image(gambar_path_temp, x=pdf.l_margin, w=180)
        ...
        pdf.set_x(pdf.l_margin)
        pdf.set_font("Helvetica", "B", 12)
        pdf.multi_cell(0, 8, bersihkan_teks_pdf("Rekomendasi"))
        pdf.set_x(pdf.l_margin)
        pdf.set_font("Helvetica", "", 11)
