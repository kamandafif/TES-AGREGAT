...
generate_data.py
-----------------
Skrip pembuatan dataset SINTETIS (bukan data pabrik sungguhan) untuk studi:
...
Dataset ini murni hasil simulasi statistik (bukan data lapangan) dan berfungsi
sebagai baseline / contoh data untuk menguji dashboard (Proof of Concept).

Output: data_pabrik_simulasi.csv (30 hari, interval 15 menit, 24 jam penuh
per hari -> data mentah TIDAK pernah dipotong jam kerja. Pemfilteran
"jam kerja / off-peak / 24 jam" dilakukan belakangan di sisi dashboard,
bukan di sini, supaya data dasar tetap utuh).
"""
...
# Definisi "jam kerja operasional" dipakai bersama oleh simulasi data ini
# dan filter "Konteks Operasional" di dashboard_app.py, supaya konsisten.
JAM_KERJA_MULAI = 7    # 07:00
JAM_KERJA_SELESAI = 17  # s/d < 17:00 (dipakai sbg batas atas jam kerja)


def buat_rentang_waktu():
    """Membentuk rentang waktu 30 hari dengan interval 15 menit (24 jam penuh/hari)."""
...
def simulasi_beban_mesin(waktu, rng):
    ...
    produksi pada jam kerja (07.00-17.00) sehingga rentang harian
    berada pada kisaran 50-90 kW saat jam operasional.
    """
    jam = waktu.hour
    n = len(waktu)
    jam_kerja = (jam >= JAM_KERJA_MULAI) & (jam < JAM_KERJA_SELESAI)
...
def simulasi_agv(waktu, seed_offset, laju_pakai, laju_isi, soc_awal=None):
    ...
    for i in range(n):
        jam = waktu[i].hour
        jam_kerja = JAM_KERJA_MULAI <= jam < JAM_KERJA_SELESAI
        ...
        if status_saat_ini:
            ...
            pemakaian = laju_pakai * (1.6 if jam_kerja else 0.4)
...
def simulasi_produksi(waktu, rng):
    ...
    n = len(waktu)
    jam_kerja = (jam >= JAM_KERJA_MULAI) & (jam < JAM_KERJA_SELESAI)
    output = np.where(jam_kerja, rng.poisson(lam=12, size=n), 0)
    return output.astype(int)


def generate_dataset():
    """Merangkai seluruh komponen simulasi menjadi satu dataframe akhir (24 jam penuh, utuh)."""
...
if __name__ == "__main__":
    ...
    print(f"Dataset sintetis berhasil dibuat: {len(df_hasil)} baris -> data_pabrik_simulasi.csv")
